...
def test(model, loader, opt, is_test=False):
    ...
    for i, data in enumerate(loader):
        ...
        # ship to device
        img_x, img_y = list(map(lambda r: r.to(device), [img_x, img_y]))
        ...
        save_image(torch.nn.functional.interpolate(img_x[0], (512, 512)), os.path.join(results_folder, "input.jpg"))
        save_image(torch.nn.functional.interpolate(img_y[0], (512, 512)), os.path.join(results_folder, "target.jpg"))
        save_image(torch.nn.functional.interpolate(pred_img[0], (512, 512)), os.path.join(results_folder, "pred.jpg"))
